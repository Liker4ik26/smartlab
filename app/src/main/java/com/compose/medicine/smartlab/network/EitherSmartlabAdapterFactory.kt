...
internal class EitherSmartlabAdapterFactory @Inject constructor() : CallAdapter.Factory() {
    override fun get(
        returnType: Type,
        ...
        val errorBodyConverter = retrofit.responseBodyConverter<SmartlabErrorNetwork>(
            SmartlabErrorNetwork::class.java,
            annotations
        )
    ...
    private class ResultCallAdapter<R, T>(
        ...
        private val errorBodyConverter: Converter<ResponseBody, SmartlabErrorNetwork>
    ) :
    ...
    private class ResultCallWrapper<F, T>(
        ...
        private fun getReasonByCode(
            response: Response<T>,
            errorResponse: SmartlabErrorNetwork
        ) = when (response.code()) {
            404 -> ErrorReason.NotFound(errorResponse.details)
            else -> ErrorReason.ServerError(errorResponse.details)
        }
    }
}